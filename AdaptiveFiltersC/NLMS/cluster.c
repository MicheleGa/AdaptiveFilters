...
// NLMS data structures
float w[LENGTH] = W_NLMS_INIT;
float w_check[LENGTH] = W_NLMS_CHECK;
// X is a known driving signal
float x[N_SAMPLES] = NLMS_X;
// input is the input signal with some noise added
float input[N_SAMPLES] = NLMS_INPUT;
float error[N_SAMPLES];
float error_check[N_SAMPLES] = NLMS_ERROR;

float filter_w[LENGTH];
float filter_w_check[LENGTH] = FINAL_NLMS_FILTER_W;
